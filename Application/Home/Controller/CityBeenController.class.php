...
class CityBeenController extends Controller {
	...
	public function been(){
		if(IS_AJAX){
			if(session('?login')){
				...
				$list = $CityBeenModel->checkByCityIdAndUserId($city_id,$user_id);
				...
				if(count($list)==0){//如果为第一次点赞
					...
					$result = $CityBeenModel->where("id=".$list[0]['id'])->save($data);
					$dynamicsResult = 1;
				}
