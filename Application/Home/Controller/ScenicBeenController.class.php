...
class ScenicBeenController extends Controller {
	...
	public function been(){
		if(IS_AJAX){
			if(session('?login')){
				...
				$ScenicBeenModel = D('ScenicBeen');
				$list = $ScenicBeenModel->checkByScenicIdAndUserId($scenic_id,$user_id);
				...
				if(count($list)==0){//如果为第一次点赞
					...
					$data['time'] = $time;
					$result = $ScenicBeenModel->add($data);
					...
					// 18-话题点赞ID
					//添加动态
					$id = $ScenicBeenModel->getLastInsID();
					$Data['type'] = 9;
					$Data['user_id'] = $user_id;
					$Data['content_id'] = $id;
					$Data['time'] = $time;
					...
					$result = $ScenicBeenModel->where("id=".$list[0]['id'])->save($data);
					$dynamicsResult = 1;
				}
