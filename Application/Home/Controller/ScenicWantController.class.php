...
class ScenicWantController extends Controller {
	...
	public function want(){
		if(IS_AJAX){
			if(session('?login')){
				...
				$ScenicWantModel = D('ScenicWant');
				$list = $ScenicWantModel->checkByScenicIdAndUserId($scenic_id,$user_id);
				...
				if(count($list)==0){//如果为第一次点赞
					...
					$data['time'] = $time;
					$result = $ScenicWantModel->add($data);
					...
					// 18-话题点赞ID
					//添加动态
					$id = $ScenicWantModel->getLastInsID();
					$Data['type'] = 8;
					$Data['user_id'] = $user_id;
					$Data['content_id'] = $id;
					$Data['time'] = $time;
					...
					$result = $ScenicWantModel->where("id=".$list[0]['id'])->save($data);
					$dynamicsResult = 1;
				}
