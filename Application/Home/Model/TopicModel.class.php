...
class TopicModel extends RelationModel{
	...
	public function getList($type,$page,$count){
		...
		$List = $M->table($DB_PREFIX.'topic_type_belong ttb,'.$DB_PREFIX.'topic t')
				  ...
				  ->where('t.id = ttb.topic_id and t.delete_tag = 0 '.$condition)
	...
	public function getAllCount($type){
		...
		if((int)$type!=0)
			$condition = 'and ttb.type_id ='.$type;
		$result = $model->query("select count(distinct ttb.topic_id) count from ".$DB_PREFIX."topic_type_belong ttb,".$DB_PREFIX."topic t where t.id = ttb.topic_id and t.delete_tag = 0 ".$condition);
